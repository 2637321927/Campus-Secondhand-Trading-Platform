...
public class AppealController : ControllerBase
{
    ...
    [HttpPost("{appealId:long}/attachments")] public async Task<ActionResult<WorkOrderDto>> Attachment(long appealId,IFormFile file){var w=await Own(appealId);if(w==null)return NotFound();if(file==null||file.Length==0)return BadRequest("附件不能为空");var uploaded=await _files.UploadMultipleAsync(new List<IFormFile>{file},Uid);w.Info=((w.Info??string.Empty)+$"\n[附件:{uploaded.Single().FileId}:{uploaded.Single().FileName}]").Trim();_orders.Update(w);await _orders.SaveAsync();return Ok(Map(w));}
