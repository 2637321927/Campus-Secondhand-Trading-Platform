...
async function loadOrders(): Promise<void> {
    ...
    try {
        const response = await getBuyingOrders()
        orderList.value = response.data ?? []
        await loadOrderImages(orderList.value)
    } catch (error) {
        orderList.value = []
        errorMessage.value = getApiErrorMessage(error, '订单列表加载失败，请稍后重试')
        console.error('购买订单加载失败：', error)
    } finally {
        loading.value = false
    }
}
    ...
    <main class="order-list-page">
        <div class="order-list-container">
            ...
            <header class="page-header">
                <p class="page-eyebrow">MY PURCHASES</p>
                <h1>我购买的订单</h1>
                <p class="page-description">
                    查看你的购买订单状态，跟踪发货和收货进度。
                </p>
            </header>
            ...
            <el-empty
                ...
                description="暂无购买订单"
            >
                <el-button type="primary" plain @click="router.push({ name: 'product-list' })">
                    去逛逛
                </el-button>
            </el-empty>
